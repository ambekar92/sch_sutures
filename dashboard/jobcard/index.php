 ...
 saveJobCard:function(){
  debugger;
      ...
      var remarks = $('#remarks').val();
		  var plan_type = $('#plan_type').val();
      //var total_qty_yield = $('#total_qty_yield').val();
...
viewQRCode:function ()
{
  ...
  params += ', fullscreen=yes,scrollbars: 0';
   ...
   var baseUrl ="http://<?php echo $_SERVER['HTTP_HOST']; ?>/sch_sutures/dashboard/jobcard/jobQRcode.php?code="+customer_code+"&date="+requireDate+"&fg="+size_fg;
...
getQtyValYield:function(val){
  if(val>175){
    alert('Yield will not be more 175 Dozen.');
    $('#total_qty_yield').val(0);
    $('#msgQtyYield').html('');
  }else{
    $('#msgQtyYield').html('');
    var totalQty=val*12;
     yieldRes = (2100 - totalQty);
    $('#msgQtyYield').html('Total Yield : <b> 2100 -'+totalQty+' = '+yieldRes+'</b>');
  }
    ...
    var myData = {getViewJobCardData:'getViewJobCardData', customer_code:customer_code,requireDate:requireDate,size_fg:size_fg,plant_id:plant_id};
      $.ajax({
        ...
        if(obj.jobPoDetails==null){
         ...
         // alert((globalJobPOData[0].total_qty)/12);
          $('#plan').val(globalJobPOData[0].plan);
          $('#plan_type').val(globalJobPOData[0].plan_code).change();
          $('#remarks').val(globalJobPOData[0].remarks);
          $('#total_qty_yield').val((globalJobPOData[0].total_qty_yield)/12);
          $('#msgQty').html('');
          var totalQty=(globalJobPOData[0].total_qty);
          $('#msgQty').html('Total Qty : <b>'+totalQty+'</b>');

          var totalQtyYield=(globalJobPOData[0].total_qty_yield);
         // alert(totalQtyYield);
          //$('#msgQtyYield').html('Total Qty : <b>'+totalQty+'</b>');
          //var totalQtyVal=totalQtyYield;
          yieldRes = (2100 - totalQtyYield);
          $('#msgQtyYield').html('Total Yield : <b> 2100 -'+totalQtyYield+' = '+yieldRes+'</b>');
            ...
            <div class="form-group">
             ...
             <div class="row">
              ...
              <div class="row">
                ...
                <div class="col-md-6">
                  <label class="control-label col-md-4 col-sm-6 col-xs-12">Total Yield
                     <sub> (Dozen) </sub><span class="required">*</span></label>
                    <div class="col-md-6 col-sm-6 col-xs-12">
                      <input type="number" name="total_qty_yield" id="total_qty_yield" placeholder="Total Yield" 
                      class="form-control" required="true" min="0" max="175" oninput ="this.value = Math.abs(this.value)" onkeyup="tempData.jobcard.getQtyValYield(this.value);"/>
                      <p style="font-size: 13px;margin-top: 1%;" id="msgQtyYield"></p>
                    </div>
                </div>
